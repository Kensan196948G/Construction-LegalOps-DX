...
async def _seed_contract(db_session) -> int:
...
async def test_start_end_list_hold(db_session) -> None:
    contract_id = await _seed_contract(db_session)
    ...
    assert await legal_hold_service.is_under_legal_hold(
    ...
    ok = await legal_hold_service.end_legal_hold(
        db_session, hold_id=int(hold.id), actor_id=1
    )
    assert ok is True
    assert await legal_hold_service.is_under_legal_hold(
        db_session, contract_id=contract_id
...
async def test_duplicate_active_hold_raises(db_session) -> None:
    ...
    await legal_hold_service.start_legal_hold(
        db_session, contract_id=contract_id, reason="1件目", requested_by=1
    )
    with pytest.raises(legal_hold_service.LegalHoldError):
        await legal_hold_service.start_legal_hold(
            db_session, contract_id=contract_id, reason="2件目", requested_by=1
        )


async def test_end_hold_returns_false_when_missing(db_session) -> None:
    assert (
        await legal_hold_service.end_legal_hold(
            db_session, hold_id=999999, actor_id=1
        )
        is False
    )
