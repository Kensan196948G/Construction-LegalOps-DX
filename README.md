...
## 🚦 現在のリリース直前状態 (2026-08-12 / Loop 110)

| 項目                           | 現在状態                                         | 補足                                                                                                                                                                                                                                                                                                                 |
| ------------------------------ | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📋 総合評価・改善 (2026-08-12) | 条件付き利用可（75.2→76.2〜76.5・代替率 61→65%） | [`docs/EVALUATION_IMPROVEMENT_REPORT_2026-08-12.md`](./docs/EVALUATION_IMPROVEMENT_REPORT_2026-08-12.md) に 18 次元評価・競合比較・ロードマップを記載。契約編集・法務相談 RAG・期限実データ・アプリ層レート制限等を実装済み                                                                                          |
...
### ✍️ 契約ライフサイクル完備（Phase 1・2026-09-05 対応 F1〜F7）

- 🔍 **契約書全文検索**（`/search`）: 契約メタデータ・条項本文・文書を横断検索し、ヒット位置スニペット付きで表示
- ✍️ **電子契約・電子署名**（`/signing`）: draft→送信→承諾証跡→署名→締結をルールエンジンで管理（建設業法 19 条の電磁的方法・承諾証跡対応・CloudSign/DocuSign は fail-closed）
- 🤝 **契約交渉・Redline 管理**（`/negotiations`）: 条項ステータス（合意/拒否/交渉中）・担当オーナー（法務/工事/営業/購買）・交渉履歴タイムライン
- 📅 **契約義務・Obligations Calendar**（`/obligations`）: 報告・通知・提出・保険等の義務を期限バケット（期限切れ/30 日/60 日/将来）で管理し、自動更新・解約通知期限も判定
- ⚖️ **法務案件（Matter）管理**（`/matters`）: MT-YYYY-NNNNNN 採番・状態遷移・担当アサイン・タイムライン・関係契約リンク
- 👨‍⚖️ **顧問弁護士・依頼管理**（`/outside-counsel`）: 事務所/弁護士台帳・依頼起票（LEG-採番）→ 回答 → 確認・利益相反・Confidential 分類
- 💰 **労務費基準マスタ**（`/labor-wage`）: 工種 × 都道府県 × 適用期間の履歴型基準値と、見積単価の乖離率判定（ダンピング確認）
- 🧭 **左メニュー再編**: グループ見出し（ホーム/契約/法務・建設/知財/品質・ナレッジ/管理）で 29 項目に拡張し、契約詳細から交渉・電子契約・義務へ直接遷移可能
...
# 4) テスト実行（unit + integration、既定で legalops_test に接続）
#    本番と同じ PostgreSQL エンジンで実行する（SQLite 代替は使用しない）。
#    接続先を変える場合のみ PYTEST_DATABASE_URL を指定する。
