...
- 不足情報は、UIや実装上の軽微な不足情報に限り、安全かつ可逆的で合理的な暫定前提を置いて進める。
- 対象環境、resource、credential、権限、承認範囲、security、data integrity、法令または契約に関する不足情報は前提で補わず、検証不能なら停止する。
- 対象を一意に特定できない場合、権限不足、Unknown Impact、破壊的変更または承認範囲外変更の可能性がある場合も停止条件とする。
...
- 変更規模にかかわらず、lint、unit test、integration test、build、security check、reviewを必須ゲートとして扱う。
- 変更範囲に比例して、API、E2E、migration、backup/restore、performance、accessibility、failure-path testを追加実行する。
- 失敗した検証は、原因を特定して修正後に再実行する。
- 実行できない検証は成功扱いにせず、`NOT RUN`または`BLOCKED`として理由、代替確認、残作業を記録する。

### Review

- correctness、security、maintainability、performance、accessibility、operationsを確認する。
- レビュー指摘ごとに重要度、採用判断、理由、対応および検証結果を記録する。
- 未レビューまたは未検証の変更を完了扱いにしない。

### Improvement

- 発見した問題を再発防止策、テスト、文書または自動化へ反映する。
- 改善効果が小さい反復を無制限に続けず、完了条件とリスクから終了を判断する。
- 同一原因のエラーは、1回目は最小修復、2回目は原因分析と再発防止、3回目はBlockedとして停止し、同じ失敗への無限修復を禁止する。
...
次を必須品質ゲートとして確認する。ツールや環境の制約で実行できない場合も成功扱いにせず、`NOT RUN`または`BLOCKED`として理由を記録する。

- formatterおよびlintが成功している。
- typecheckが成功している。
- unit testが成功している。
- integration testが成功している。
- APIおよびE2Eテストが必要範囲で成功している。
- production相当buildが成功している。
- security checkとreviewが完了している。
...
- 必須機能が実装済み
- 適用範囲のformat、lint、typecheck、unit/integration/API/E2E test、build、security checkおよびreviewがすべて`PASS`
...
7. 人間実行後にdeployment ID、commit SHA、migration結果および時刻をread-onlyで確認・記録
...
5. PR記載範囲内のread-only確認、既存artifactの確認、承認済みCI/CD結果の確認
6. PR記載範囲外のソース、設定、DB、認証認可の変更・修正・回帰テスト・再反映は自律実行せず、新たなPRまたはApproval PRと明示承認を必須とする
7. 定義済み条件該当時の事前検証済みrollback手順生成と、人間または事前承認済みCI/CDによる実行後のread-only確認
8. rollback後の再確認と無制限な再デプロイの禁止
9. Issue、Project、release note、runbookおよび既知の問題の更新
10. 最終報告
...
- Critical Security Issueを検出した場合は通常開発を即時停止し、封じ込め、原因分析、影響範囲確認、rollback判断を優先する。
...
対象環境不明、権限不足、必須検証失敗、安全なbackup・rollback不能、破壊的migration、承認範囲外変更、データ損失の危険がある場合のみ安全に停止し、理由、影響、代替案、再開条件を報告してください。stop after 20 turns に到達した場合も、実績、未完了項目、残存リスク、次回再開条件を報告して停止してください。本番データ削除、force push、保護回避、課金・DNS・認証・Secretsの無断変更は禁止します。
