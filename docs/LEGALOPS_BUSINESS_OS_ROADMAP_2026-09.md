...
> 状態: **承認済み（2026-09-04・PR #96 squash merge `8929793`）** — この文書が Phase 1〜5 の実装計画の正本です。
...
### Phase 2: 建設業特化を完成（🔴 / 🟠）— ✅ **完了（2026-09-05・P0 全項目 + P1 主要項目実装）**
...
- 進捗（2026-09-05）:
  ...
  - ✅ #27 見積書様式生成（`/labor-wage/estimate-form`・総括表＋明細表・内訳整合検証）
  - ✅ #28 コミットメント条項管理（`labor_commitments`・migration 021・表明→履行/違反確認）
  - 🚧 残（後続 Phase へ繰り越し）: 公共工事 P1 の外部連携系（#44-53・#58-59・
    e-BISC/GECS 等は外部 API 連携のため人間ゲート後）
- DoD: 更新型 Compliance Engine（データ取込 1 系統以上をデモモードで実証）＋公共工事ダッシュボード

### Phase 3: リスク統制（🔴 / 🟠）— ✅ **完了（2026-09-06・Issue #121〜#124）**
...
- 進捗（2026-09-06）:
  - ✅ #97-112 紛争・クレーム管理高度化（`dispute_delay_events` /
    `dispute_argument_positions` / `dispute_settlement_options` /
    `dispute_proceeding_stages`・migration 022・`/disputes/[id]`）
  - ✅ #113-124 独禁法・入札談合コンプライアンス（`antitrust_checks` /
    `antitrust_prior_applications` / `antitrust_consultations` /
    `compliance_trainings`・migration 023・`/compliance/antitrust`）
  - ✅ #125-135 内部通報・調査管理（`whistleblower_reports` /
    `whistleblower_reporter_profiles`（識別情報分離）/ `whistleblower_case_access` /
    `whistleblower_evidence` / `whistleblower_interviews` /
    `whistleblower_timeline_events` / `whistleblower_actions`・migration 024・
    `/whistleblower`）※ 通報者情報隔離を RLS + ACL で実装、単体・統合テストで検証済み
  - ✅ #217-230 証拠・eDiscovery 管理（`evidences` / `evidence_custody_events` /
    `evidence_hold_release_approvals`・migration 025・`/evidence`）
  - DoD 達成根拠: backend pytest 1367 passed / ruff・mypy clean / alembic
    upgrade→downgrade→re-upgrade（実PostgreSQL）成功 / frontend typecheck・lint
    clean・jest 93 passed・production build成功
  - 既知の残課題: [#125](https://github.com/Kensan196948G/Construction-LegalOps-DX/issues/125)
    （既存 `LegalHold.started_at` の `server_default` 文字列指定の潜在バグ、
    Issue #124 実装中に発見・別Issue化）
